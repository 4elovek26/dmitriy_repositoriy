...
Работа с файлами
$file = fopen(__DIR__ . '...', 'r') - открывает файл/// fopen(путь к файлу, параметр открытия) r- для чтения, w - для записи
file_exists(путь к файлу); - проверка на существование
is_writable(путь к файлу); - проверка на существование и возможность записи
fwrite($file, "hello textfile"); запись
fclose($file); закрытие
PHP_EOL - перенос строки
file_put_contents(путь к файлу, текст, FILE_APPEND(добавление строки, а не перезапись)); fopen+fwrite+fclose
fread($file, количество байт для считывания(filesize(путь к файлу)); чтение файла, можно вывести данные на экран
fgets($file) -считывает строку файла(начиная с первой)
feof($file) - пробегается по строкам файла, пока не достигнет конца
file_get_contents(путь к файлу)



КУКИ
установить куки через php
setcookie(name, value, время жизни(time()+3600=час));
$_COOKIE
*/
